...
- 새 글은 `docs/content-templates/`의 틀(프로젝트·스터디·세미나)을 복사해 씁니다. 쓰는 법은 그 폴더의 `README.md`. 예시 글(`sample-*`)은 2026-09-23에 모두 지웠고, 빈 컬렉션 폴더는 `.gitkeep`으로 남겼습니다 — 스터디·세미나에 첫 글이 생기기 전까지 빌드 때 "컬렉션이 비었다" 경고가 나오는 것은 정상입니다. 주요 기능 `description`은 Markdown이 아니라 일반 텍스트로 나옵니다.
...
- 북마크 카드: 글에 **주소만 한 문단으로** 쓰면(맨 URL·`<https://…>`) `src/lib/bookmark.ts`(Sätteri hast 플러그인, `astro.config.mjs`의 `markdown.processor`)가 빌드 때 카드로 바꿉니다. `[글자](주소)`는 그대로 링크입니다. OG 정보는 `src/lib/linkPreview.ts`가 가져와 `src/data/link-previews.json`에 캐시하고 **이 파일을 커밋**합니다(다시 가져오려면 항목을 지우고 빌드). 스타일은 `prose.css`의 `.bookmark`. 근거는 `PROGRESS.md` 5-22.
