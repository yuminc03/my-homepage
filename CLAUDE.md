...
- 새 글은 `docs/content-templates/`의 틀(프로젝트·스터디·세미나)을 복사해 씁니다. 쓰는 법은 그 폴더의 `README.md`. 예시 글(`sample-*`)은 2026-09-23에 모두 지웠고, 빈 컬렉션 폴더는 `.gitkeep`으로 남겼습니다 — 스터디에 첫 글이 생기기 전까지 빌드 때 "컬렉션이 비었다" 경고가 나오는 것은 정상입니다(세미나는 2026-09-23부터 글이 있습니다). 사용자가 Notion에 남긴 재료를 읽는 방법은 `PROGRESS.md` 2-7이고, **행사 사진은 1~2장(많아야 3~4장)만** 넣습니다. 세미나 재료를 받는 양식은 `docs/content-templates/seminar-input.md`입니다 — 특히 세션마다 **"공개하면 안 되는 것"**(발표자의 미공개 수치·사내 사례 등)을 받아 글에서 뺍니다. 주요 기능 `description`은 Markdown이 아니라 일반 텍스트로 나옵니다.
