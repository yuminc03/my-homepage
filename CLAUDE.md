...
- Astro 프로젝트를 저장소 루트에 생성했습니다. 공통 셸(메뉴바·Dock 자동 숨김·창·테마 버튼·시계)과 목록 페이지 틀 3개까지 `develop`에 병합·원격 push했고(2026-09-15), 지금은 `feature/home-page`에서 홈을 구현 중입니다(첫 화면·iPhone 목업 완료, 최근 기록 남음). 페이지 순서는 홈 → 프로젝트 → 스터디 → 세미나입니다. 배포 주소는 GitHub Pages 하위 경로 `https://yuminc03.github.io/my-homepage/`입니다.
