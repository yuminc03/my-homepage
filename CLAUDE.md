...
- 저장소가 iCloud로 동기화되는 `~/Documents` 아래에 있어 `파일 2.확장자`·`폴더 2` 같은 사본이 생길 수 있습니다. 커밋하지 말고 **보이면 묻지 않고 지웁니다**(2026-09-22 사용자 결정, `node_modules` 안이면 지우고 `npm ci`). `PROGRESS.md` 2-2 끝.
...
- 새 글은 `docs/content-templates/`의 틀(프로젝트·스터디·세미나)을 복사해 씁니다. 쓰는 법은 그 폴더의 `README.md`. 예시 글(`sample-*`)은 2026-09-23에 모두 지웠고, 빈 컬렉션 폴더는 `.gitkeep`으로 남겼습니다 — 스터디에 첫 글이 생기기 전까지 빌드 때 "컬렉션이 비었다" 경고가 나오는 것은 정상입니다(세미나는 2026-09-23부터 글이 있습니다). 주요 기능 `description`은 Markdown이 아니라 일반 텍스트로 나옵니다.
- 사용자에게 **재료를 받을 때는 `docs/content-templates/`의 재료 양식**을 안내합니다 — 세미나는 `seminar-input.md`, 스터디는 `study-input.md`. 다 채우지 않아도 되고, 꼭 필요한 칸은 **"공개하면 안 되는 것"** 하나입니다(발표자의 미공개 수치·회사 코드 등 — 필기만 봐서는 판단할 수 없습니다). **소감·배운 점은 선택**이라 비어 있으면 본문에서 초안을 써서 보여 주고 사용자가 고칩니다. 사진은 **세미나 2~3장(많아야 4장)**, **스터디는 없어도 됩니다(있으면 1~2장)**. 고르고 자르고 줄이는 것은 이쪽에서 합니다. 사용자가 Notion에 남긴 재료를 읽는 방법은 `PROGRESS.md` 2-7입니다(`app.notion.com`은 못 읽고 `notion.site` 게시 주소가 필요합니다).
...
- 공유 카드(OG): `BaseLayout`이 canonical·`og:*`·`twitter:card`를 넣습니다. 상세 화면은 `SiteLayout`에 `title`을 넘기면 `article` + 글 제목으로, 썸네일을 바꾸려면 `shareImage`(`{ url, width, height, alt }`)를 넘깁니다(세미나 상세가 표지 사진으로 씀). 주소는 반드시 `Astro.site` 기준 절대 주소입니다. 대표 이미지 `public/og-image.png`는 `design/og-image.html`(사이트 `tokens.css`를 불러옴)을 헤드리스 Chrome으로 찍은 것이라 **토큰·아이콘을 바꾸면 다시 찍습니다**. 근거는 `PROGRESS.md` 5-23.
- 북마크 카드: 글에 **주소만 한 문단으로** 쓰면(맨 URL·`<https://…>`) `src/lib/bookmark.ts`(Sätteri hast 플러그인, `astro.config.mjs`의 `markdown.processor`)가 빌드 때 카드로 바꿉니다. `[글자](주소)`는 그대로 링크입니다. OG 정보는 `src/lib/linkPreview.ts`가 가져와 `src/data/link-previews.json`에 캐시하고 **이 파일을 반드시 커밋**합니다(다시 가져오려면 항목을 지우고 빌드). 커밋하지 않으면 CI가 빌드할 때 다시 가져오고, 실패하면 카드가 조용히 맨 링크로 나옵니다. 스타일은 `prose.css`의 `.bookmark`. 근거는 `PROGRESS.md` 5-22.
- **Markdown 처리 방식을 바꿨거나 글·이미지를 지웠다면 `node_modules/.astro`를 지우고 빌드합니다.** 콘텐츠 컬렉션의 렌더 결과가 캐시되어 예전 HTML이 그대로 나오고, 지운 글의 이미지를 계속 찾아 `ImageNotFound`로 빌드가 실패합니다(2026-09-23 예시 글 삭제 때 겪음, CI는 캐시가 없어 영향 없음).
...
- 세미나: 목록 `src/pages/seminars/index.astro`(항목 `SeminarCard` — DOM 한 벌로 모바일 세로 카드 / 태블릿 가로 카드 / 데스크톱 타임라인), 상세 `src/pages/seminars/[slug].astro`(주소는 `seminarHref()`). 표지는 `SeminarCover`, 본문 사진은 `Photo`·`PhotoPair`·`PhotoSide`이고 상세 페이지가 `<Content components={{ ... }} />`로 넘긴다. 세션 머리는 `## 라벨` + `### 제목` 한 쌍이며 CSS `h2:has(+ h3)`로 라벨과 마무리 구획 제목을 가른다. 결정 근거는 `PROGRESS.md` 5-16.
- 세미나의 세션·주최(`PROGRESS.md` 5-25): 프런트매터 `host`는 선택이고 상세 머리·목록 카드·검색 색인에 나온다. **주최가 두 곳 이상일 때만** 목록에 `FilterChips`가 나온다(스터디 목록과 같은 조건). **세션 목록은 프런트매터에 적지 않는다** — `src/lib/sessions.ts`의 `getSeminarSessions(headings)`가 `render()`의 소제목에서 `depth 2` 바로 뒤에 `depth 3`이 오는 쌍만 뽑는다(본문 CSS `h2:has(+ h3)`와 같은 기준). 라벨은 `## SESSION 01 · 발표자 · 15분`이고 세 번째 칸은 선택, 번호가 없으면 순서대로 매긴다. 상세는 표지 아래 `SessionList.astro`로 그리고, 목록 페이지는 같은 함수로 세션 수를 센다(글마다 `render()`, 빌드 때 한 번). 앵커는 라벨의 slug이고 본문 `h2`·`h3`에 `scroll-margin-top: 80px`이 필요하다.
