...
- **마지막 세션(2026-09-24, 이어서)**: **사용자와 WWDC26 글을 함께 확인하고 커밋했다.** 고친 것: 개발자 스토리에서 **발표 제목을 빼고 시연을 "영상 편집 앱에서"로 흐렸다**(회사를 짐작할 수 없게, 따옴표 안의 실제 명령어도 뺐다) · `AutoLayout` → **`Auto Layout`** · `entity` → **`Entity`**로 통일 · `appTransaction` → **`AppTransaction` 안의 `appTransactionID`** · Xcode 문단에 주어를 넣고 계획 모드를 "**Xcode가 먼저 수정하고 개발자가 검토하는 방식**"으로(사용자 설명) · 굿즈는 노트가 아니라 **책 모양 핀 배지 종이 케이스**라 대체 텍스트·캡션을 고치고 파일을 `photo-case.jpg`로 바꿨다 · 마지막 문단은 "이날 발표는 녹화본이 없지만 WWDC26 세션 영상을 참고하면 된다"로. **그대로 둔 것**: MLX 문장(사용자 필기), 제목의 `(오프라인)`(공식 행사명). 커밋은 **`110ccc4` WWDC26 글 → `1b2993f` letswift 워터마크·`aspect`** 두 개(같은 브랜치 `feature/seminar-idea-playground`). **`develop` 병합·배포는 아직**이다
  ...
  - 사용자는 **다음 세미나(Apple PR 이벤트) 재료를 검토하는 중**이다
...
3. **2026-09-23 배포 완료**: 글꼴 직접 제공(5-24) · 예시 글 정리 · Pointer Quest 진행 기간 · 세미나 양식 확장(5-25) · 첫 세미나 기록을 `develop` → `master` 병합 `dc3db99`로 한 번에 배포했다(run 35821441390 성공). 실제 주소에서 홈·세미나 목록·행사 상세·검색 색인·프로젝트 상세 200, 상세의 세션 한눈에 보기·주최·세션 수·북마크 카드·표지 OG를 확인했다. (그 전 배포는 2026-09-22 `81b7ac3`)
   ...
   - **다음에 할 일(사용자 승인 순서)**
     ...
     2. **실제 콘텐츠 채우기**(2-6) ← **지금 여기.** 세미나 기록 2개(WWDC26 하이라이트 · 아이디어 플레이그라운드)를 썼고 사진 워터마크(2-9)·사진 칸 비율 옵션(5-26)까지 끝냈다. 지금 브랜치는 **`feature/seminar-idea-playground`**, 커밋은 `a7f91be`(사진 칸 비율) · `bdeae62`(아이디어 플레이그라운드) · `110ccc4`(WWDC26) · `1b2993f`(letswift 워터마크)이고 **두 글 모두 사용자 확인을 마쳤다.** 남은 것은 **`develop` 병합 → `master` 병합(배포)** 이다(사용자에게 확인받고 진행). 다음 재료는 `seminar_apple_pr_event/`(Apple Korea PR 프라이빗 이벤트)에 이미 들어와 있다. 재료가 오면 `docs/content-templates/`의 **재료 양식**(`seminar-input.md` · `study-input.md`)에 맞춰 읽어 글 틀에 채운다. Notion 링크면 **2-7**, 사진 처리는 **2-8**, 워터마크는 **2-9**. **스터디는 아직 글이 하나도 없다**
...
- [ ] **세미나 양식 확장**: 주최 `host`·세션 한눈에 보기·카드의 세션 수(2026-09-23 결정, **5-25**) — 사용자가 세미나 내용을 넣을 때 `feature/seminar-sessions`로 구현
- [ ] 다음 배포: `develop` → `master`(글꼴 직접 제공·예시 글 정리가 아직 `develop`에만 있다)
...
## 12. 브랜치·커밋 기록
- 로컬 브랜치: `master`, `develop`(현재). 기능 브랜치는 없다. 원격(`origin`, `https://github.com/yuminc03/my-homepage.git`, **2026-09-21부터 공개 저장소**): `master` `dc3db99`(2026-09-23 배포, 로컬과 같음), `develop`(2026-09-23 push, `origin/develop` 추적). **`master`에 push하면 GitHub Actions가 배포한다**(5-20)
