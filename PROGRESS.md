# 진행 상황
- 최종 업데이트: 2026-09-24 (세미나 기록 4개 완성, 브랜치 `feature/seminar-apple-pr-event`, **다음은 이 브랜치 `develop` 병합 → 배포**)
- 이 문서 하나만 읽으면 새 채팅에서 바로 이어서 작업할 수 있도록 정리한 단일 기준 문서다
- **새 채팅은 여기부터**: 지금 브랜치는 **`feature/seminar-apple-pr-event`**(`develop` `c2ac9e0`에서 분기)이고 커밋 `b9b0039`(Apple PR 이벤트 글) + 문서 커밋이 있다. 작업 트리는 깨끗하다(재료 폴더 `seminar_apple_pr_event/`는 2026-09-24 사용자 요청으로 지웠다). 원격 push는 이번 세션에 하지 않았다 — `develop`은 `origin/develop`보다 앞서 있다. **다음 할 일: ① 사용자 확인 후 `feature/seminar-apple-pr-event` → `develop` 병합·브랜치 삭제 ② `develop` push ③ `develop` → `master` 병합 push로 배포**(세미나 3개 · 사진 칸 비율 · 워터마크가 아직 배포 전) ④ 실제 주소에서 세미나 목록·상세 4개 확인. 그 뒤는 다음 콘텐츠(스터디는 아직 0개)
- **마지막 세션(2026-09-24, 이어서)**: **WWDC26 글을 사용자와 확인·커밋하고, Apple Korea PR 프라이빗 이벤트 글을 써서 커밋했다.** 중간에 `feature/seminar-idea-playground`를 `develop`에 병합(`c2ac9e0`)·삭제하고 `feature/seminar-apple-pr-event`를 새로 땄다(두 브랜치가 모두 `PROGRESS.md`를 고쳐 충돌하지 않게 먼저 병합했다). 아래는 WWDC26 확인 내용. 고친 것: 개발자 스토리에서 **발표 제목을 빼고 시연을 "영상 편집 앱에서"로 흐렸다**(회사를 짐작할 수 없게, 따옴표 안의 실제 명령어도 뺐다) · `AutoLayout` → **`Auto Layout`** · `entity` → **`Entity`**로 통일 · `appTransaction` → **`AppTransaction` 안의 `appTransactionID`** · Xcode 문단에 주어를 넣고 계획 모드를 "**Xcode가 먼저 수정하고 개발자가 검토하는 방식**"으로(사용자 설명) · 굿즈는 노트가 아니라 **책 모양 핀 배지 종이 케이스**라 대체 텍스트·캡션을 고치고 파일을 `photo-case.jpg`로 바꿨다 · 마지막 문단은 "이날 발표는 녹화본이 없지만 WWDC26 세션 영상을 참고하면 된다"로. **그대로 둔 것**: MLX 문장(사용자 필기), 제목의 `(오프라인)`(공식 행사명). 커밋은 **`110ccc4` WWDC26 글 → `1b2993f` letswift 워터마크·`aspect`** 두 개(같은 브랜치 `feature/seminar-idea-playground`). **`develop` 병합·배포는 아직**이다
  - **`.gitignore`의 `/seminar_*/`는 넣지 않기로 했다**(2026-09-24 사용자 결정 — 나중에 `seminar`로 시작하는 폴더가 저장소에 생길 수 있다). 되돌렸다. 대신 재료 폴더 `seminar_apple_pr_event/`는 글을 쓴 뒤 **사용자가 지운다**. 그때까지 `git status`에 `??`로 보이니 **`git add .`·`git add -A`를 쓰지 말고 경로를 지정해 스테이징한다**
  - Astro 7은 **`astro dev`도 백그라운드로 분리되어 계속 떠 있다**(`preview`와 같다). 끄려면 `npx astro dev stop`, 상태는 `npx astro dev status`
  - **Apple Korea PR 프라이빗 이벤트 글**(`src/content/seminars/apple-korea-pr-private-event/`, 2026-07-29, 커밋 `b9b0039`, **사용자 확인 완료**). 재료는 `seminar_apple_pr_event/`의 재료 양식 md(사용자가 "내용 괜찮다"고 확인)와 참가 확정 메일 스크린샷 한 장. 정한 것: ① 메일에 **"관계자 얼굴 사진·이름을 옮기지 말라"** 는 안내가 있어 발표자 닉네임 4개를 모두 **`수료생`** 으로 적었다(사용자: 있어도 빼도 상관없으니 그대로) — 앱 이름 **모스픽은 남긴다**(사용자 결정) ② 재료의 "스페셜 컴퓨팅"은 **공간 컴퓨팅(Spatial Computing)**, `VisionOS`는 **`visionOS`** 로 고쳤다 ③ 주최는 초대 메일을 보낸 **`Apple 디벨로퍼 아카데미 @ POSTECH`**(아이디어 플레이그라운드와 같은 값 → 목록 필터에서 한 곳으로 묶인다) ④ 세션별 시간이 없어 라벨 셋째 칸은 비웠다(전체 18:00~19:30은 도입 문단) ⑤ 사진이 메일 한 장뿐이라 **표지는 메일에서 로고·제목·일정 두 줄만 잘라 흰 4:3(1600×1200) 판에 다시 배치**했다(사용자: 그대로 좋다) — 인사말의 실명 줄은 넣지 않았고, 워터마크는 WWDC26 표지와 같은 자리·크기(판 1450–1565 × 757–800). 본문 사진은 없다 ⑥ "발표 말고 다른 순서"는 `## Custom Built Community`(`###` 없음 → 세션으로 세지 않음, 목록은 "세션 4개") ⑦ 소감은 재료를 바탕으로 다듬었고, 해석을 덧붙인 문장 중 "기술을 먼저 고르고 쓸 곳을 찾은 발표는 하나도 없었다"는 사용자 요청으로 뺐다 ⑧ **공식 링크는 없다**(재료의 링크는 카카오맵 단축 주소라 북마크 카드로 쓰지 않는다, 사용자 확인)
- **그 전 세션(2026-09-24)**: **사진 워터마크(2-9)를 넣고, 그 과정에서 사진 칸이 세로 사진을 잘라 먹는 문제(5-26)를 찾아 고쳤다.** 사용자가 "웹사이트 이미지를 함부로 못 가져가게 하는 방법"을 물어 시작했다 — 완전히 막을 수는 없다고 설명하고(저장소가 공개라 원본도 받아진다) **워터마크**로 정했다. 강도를 세 번 다시 잡았다: 흰 글자 45%(안 보임) → 흰 글자 확대(흰 배경에서 원리적 한계) → **반투명 판 + 흰 글자**. 그래도 칸마다 보였다 안 보였다 해서 원인을 파 보니 **① `PhotoPair`·`PhotoSide`·`SeminarCover`가 `object-fit: cover`로 아래를 22~31% 잘라내고 ② 워터마크를 원본 폭 기준으로 넣어 작게 그려지는 칸에서 10px까지 줄어드는 것**이었다. ①은 `aspect` prop을 만들어 고쳤고(5-26) **이 과정에서 프로토타입 화면이 세로 38%만 남아 지도·버튼이 다 잘려 있던 것을 발견해 함께 고쳤다**, ②는 칸별 표시 폭에서 역산해 모두 18px로 맞췄다. 표지는 어떤 칸에서도 구석이 잘리므로 **모든 칸에서 살아남는 가장 아래 지점**(4:3은 세로 70%, 1.12:1은 66%)에 두기로 사용자가 미리보기를 보고 정했다. 사용자 확인 완료. 이어서 재료 폴더 `seminar_resource/`를 지우고 `.gitignore`에 `/seminar_*/`를 넣었다.
  - **커밋은 아이디어 플레이그라운드 것만 했다**(사용자 지시 — 다른 행사는 내용을 더 보겠다고 했다). 브랜치 `feature/seminar-idea-playground`(`develop` `119d4a1`에서 분기)에 **`a7f91be` 사진 칸 비율 옵션 → `bdeae62` 글·사진 6장·`link-previews.json`** 두 개. 컴포넌트를 앞 커밋으로 뗀 것은 글이 `aspect`를 쓰기 때문이고, 그래야 두 커밋이 각각 빌드된다. **`develop` 병합은 아직**이다
  - 이때 작업 트리에 남겨 둔 WWDC26 글과 letswift 워터마크는 다음 세션에서 커밋했다(위). `.gitignore` 변경은 되돌렸다
- **그 전 세션(2026-09-23, 이어서 4)**: **세미나 기록 2개를 썼다.** ① **WWDC26 하이라이트 살펴보기(오프라인)**(2026-06-26, Apple Korea 아셈타워) — 사용자가 `lonalia.notion.site` 링크를 줬고 2-7 방법으로 읽었다(본문 359블록, **chunk 4번 나눠 받아야 다 나온다**). 행사 메타데이터는 본문이 아니라 **Notion DB 속성**에 있었고(날짜·주최), 세션 제목·발표자는 **제목 슬라이드 사진**에서 확인했다. 사용자 요청으로 **발표자 이름은 모두 비공개**(`Apple`로만 표기)하고, 국내 개발사 사례 발표는 **회사·제품 내용을 빼고 「WWDC26이 바꿀 사용자 경험」 부분만** 남겼다(출시 전 기능 발표 문구는 뺐다). 세션 3개 + `## 개발자 스토리`(`###`가 없어 세션으로 세지 않는다). ② **워크숍: 아이디어 플레이그라운드 @ 올리브영N 성수**(2026-07-24, Apple 디벨로퍼 아카데미 @ POSTECH) — 사용자가 `seminar_resource/`에 재료 양식 md와 사진을 넣어 줬다. 공식 진행 순서는 사용자가 준 일정 스크린샷에서, 프로토타입 이름(**오늘동선**)과 워크숍 조건(**Privacy First · Offline Capable**)은 사진에서 찾아 보강했다. 사진 규칙은 2-8. 빌드·목록·상세·세션 한눈에 보기·북마크 카드·Pager까지 확인했고 `src/data/link-previews.json`에 아카데미 행사 항목이 추가됐다(커밋 대상). **아직 커밋 전이고, 사용자가 내용을 마지막에 손본 뒤 `develop`에 병합하기로 했다.** 저장소 루트의 `seminar_resource/`·`seminar_apple_pr_event/`는 **재료 폴더라 커밋하지 않는다**(다음 세미나 재료가 `seminar_apple_pr_event/`에 이미 들어와 있다)
- **그 전 세션(2026-09-23, 이어서 3)**: **병합·배포를 마치고 스터디 재료 양식을 만들었다.** `feature/seminar-sessions` → `develop` 병합 `b615ca9` → `master` 병합 `dc3db99`로 배포 성공(run 35821441390). 실제 주소에서 세션 한눈에 보기·주최·세션 수·북마크 카드·표지 OG·검색 색인을 확인했다. 이어서 사용자 요청으로 **스터디 재료 양식** `docs/content-templates/study-input.md`를 만들고 `docs/content-templates/README.md`에 두 양식을 함께 안내하도록 고쳤다(2-6). 사용자는 **다음 콘텐츠를 준비하는 중**이고 새 채팅에서 이어서 시작한다
- **그 전 세션(2026-09-23, 이어서 2)**: **세미나 양식 확장(5-25)을 구현하고 첫 세미나 기록을 썼다.** 사용자가 Notion 링크를 줬는데 `app.notion.com` 사설 페이지라 열리지 않아, 사용자가 **웹에 게시해 `lonalia.notion.site` 주소**로 다시 줬고 그때부터 본문·사진까지 모두 읽혔다(방법은 2-7). 브랜치 `feature/seminar-sessions`에서 주최 `host` 필드(`d8ff10e`) · 세션 한눈에 보기(`222f977`, 뽑는 함수 `src/lib/sessions.ts` + `SessionList.astro`) · 목록 카드의 주최·세션 수·주최별 필터 칩(`1681db3`)을 만들고, **10 Years of Let'Swift**(2026-04-01, Apple Korea 아셈타워) 기록을 썼다(`64f02a0`). 사진은 받은 12장 중 **3장만**(표지·행사 진행 순서·이름표) 골랐고 이름표는 방문 담당자 이름 줄을 잘라냈다. 본문에 `letswift.kr` 주소를 한 문단으로 두어 북마크 카드로 나오게 했고, **여태 커밋된 적이 없던 `src/data/link-previews.json`을 이때 처음 커밋**했다. 1440 라이트/다크 · 390 · 768 스크린샷과 빌드 HTML로 확인했다. 이어서 사용자 요청으로 **두 번째 세션의 FoundationModels 도입 앱 분석 수치를 뺐고**(발표자가 따로 진행한 밋업의 분석이라 공개 글에 옮기지 않는 편이 맞다, `3f569bc`), **세미나 재료를 넘겨받는 양식** `docs/content-templates/seminar-input.md`를 만들었다(`d2f5b26`, 소감·배운 점은 사용자 결정으로 선택 `769b28a`). **`develop` 병합 `b615ca9` → `master` 병합 `dc3db99`로 배포까지 마쳤다**(run 35821441390 성공). 실제 주소에서 세션 한눈에 보기·주최·세션 수·북마크 카드·표지 OG·검색 색인을 확인했고 미공개 수치가 빠진 것도 확인했다
...
- **Git**: 시안·기술 스택 문서·`.claude/settings.json`(`92083d0`)이 `develop`에 반영되어 있다. `feature/astro-setup`(커밋 4개)을 `develop`에 병합 `004589b` → 브랜치 삭제(2026-09-14). `feature/site-shell`(테마 버튼 `872ec4b`, 공통 셸 `20d40c8`, Dock 자동 숨김 `e8eea35`, 시계 `88e86ca`, 문서 `cfe9270`)을 `develop`에 병합 `0227fa3` → 브랜치 삭제(2026-09-15). `feature/home-page`(첫 화면 `9907ebe`, iPhone 목업 `6da0b04`, 문서 `f6741d5`, 최근 기록 `062e860`, 문서 `1f8cbb8`)를 `develop`에 병합 `8c7568f` → 브랜치 삭제 → push(2026-09-16), 뒤이어 문서 `a725134` push. `feature/project-pages`(기능 12 + 문서 3, 커밋 목록은 12장)를 `develop`에 병합 `53f528e` → 로컬·원격 브랜치 삭제 → push(2026-09-18). `feature/study-pages`(기능 9 + 문서 2, 커밋 목록은 12장)를 `develop`에 병합 `4351c43` → 브랜치 삭제 → push(2026-09-20). `feature/seminar-pages`(기능 7 + 문서 1, 커밋 목록은 12장)를 `develop`에 병합 `8f704a4` → 브랜치 삭제 → push(2026-09-20). 이어서 `feature/page-transitions`(병합 `0cd15af`) · `feature/content-search`(병합 `d81c5fc`) · `feature/deploy`(병합 `36f0456`) · `feature/site-name-favicon`(병합 `cc6040f`)까지 병합·삭제했다. 2026-09-22~23에는 `feature/real-content`(`6354a23`) · `feature/readme`(`5b8237c`) · `feature/bookmark-card`(`56027aa`) · `feature/og-meta`(`15cff8f`) · `feature/self-host-font`(`17a722e`) · `feature/content-templates`(`603b658`)를 병합·삭제했다. `feature/seminar-sessions`(`b615ca9`)까지 병합·삭제했다. 2026-09-24 `feature/seminar-idea-playground`(`c2ac9e0`)를 병합·삭제했고(**push 안 함**), 지금은 `feature/seminar-apple-pr-event`에 있다. **원격**(2026-09-23 기준, 그 뒤로 push하지 않음): `origin/develop` = 로컬 `develop`과 같음(2026-09-23 push), `origin/master` = `dc3db99`(로컬 `master`와 같음). `develop` → `master` 병합은 첫 배포 `ea31474`, Lia.log 배포 `398be50`, 콘텐츠·README·카드 배포 `81b7ac3`, 글꼴·세미나 배포 `dc3db99` 네 번 했다(12장)
- **다음 단계**: 홈 완료 → 프로젝트 목록·상세 완료(병합·push 완료) → 스터디 목록·글 완료(병합 `4351c43`, 5-15) → 세미나 목록·행사 상세 완료(병합 `8f704a4`, 5-16) + 홈 최근 기록 링크를 상세로(함께 끝냄) → 페이지 전환 모션 완료(병합 `0cd15af`, 5-17) → 콘텐츠 검색 완료(병합 `d81c5fc`, 5-19) → 배포 완료(`ea31474`, 5-20) → 사이트 이름 Lia.log·로고·파비콘 완료(`cc6040f`, 배포 `398be50`, 5-21) → Pointer Quest·README·북마크 카드·OG 완료(배포 `81b7ac3`) → 글꼴 직접 제공·예시 글 정리 완료(`develop`만) → 세미나 양식 확장·첫 세미나 기록 완료(5-25, 배포 `dc3db99`) → 세미나 기록 3개·워터마크·사진 칸 비율 완료(2026-09-24, **배포 전**) → **배포** → **실제 콘텐츠 계속**(사용자가 천천히 채움, 스터디 0개) → 실기기 확인(사용자가 보고 알려 줌) → **마지막: 개발 과정 설명 세션**(2026-09-17 요청). 화면마다 `develop`에서 `feature/*` 브랜치를 새로 만든다

## 2. 새 채팅에서 이어서 시작하기
### 2-1. 지금 바로 할 일
1. `git status`로 브랜치와 작업 트리를 확인한다
   - 기대 상태(2026-09-24 세션 끝): **`feature/seminar-apple-pr-event` 브랜치**, 마지막 커밋은 문서(`docs:`) 커밋. 작업 트리 깨끗(재료 폴더는 지웠다). 재료 폴더 `seminar_*/`는 `.gitignore`에 넣지 않았으므로 새 재료가 오면 **경로를 지정해 스테이징**한다
   - 로컬 `develop` = `c2ac9e0`(`feature/seminar-idea-playground` 병합). **`origin/develop`(`119d4a1`)보다 앞서 있고 push하지 않았다.** `origin/master` = `dc3db99`(2026-09-23 배포)
   - **배포되지 않은 것**: 사진 칸 비율 옵션(5-26) · 워터마크(2-9) · 세미나 3개(아이디어 플레이그라운드 · WWDC26 · Apple PR 이벤트). 문서만 고친 커밋은 `develop`에만 쌓는다
...
3. **2026-09-23 배포 완료**: 글꼴 직접 제공(5-24) · 예시 글 정리 · Pointer Quest 진행 기간 · 세미나 양식 확장(5-25) · 첫 세미나 기록을 `develop` → `master` 병합 `dc3db99`로 한 번에 배포했다(run 35821441390 성공). 실제 주소에서 홈·세미나 목록·행사 상세·검색 색인·프로젝트 상세 200, 상세의 세션 한눈에 보기·주최·세션 수·북마크 카드·표지 OG를 확인했다. (그 전 배포는 2026-09-22 `81b7ac3`)
   - 2026-09-22~23에 끝낸 것(모두 `dc3db99`로 배포됨): 글꼴 직접 제공(5-24) · 스터디·세미나 예시 글 삭제 + 틀은 `docs/content-templates/`로 · Pointer Quest 진행 기간 · **세미나 양식 확장과 첫 세미나 기록**(5-25). 배포 뒤 **재료 양식 2종**(`seminar-input.md`·`study-input.md`)을 문서로 추가했다(2-6)
   - **다음에 할 일(사용자 승인 순서)**
     1. (끝남) **세미나 양식 확장(5-25)과 첫 세미나 기록** — 구현·작성·병합·배포까지 끝났다(2026-09-23, `dc3db99`)
     2. **실제 콘텐츠 채우기**(2-6) ← **지금 여기.** 세미나 기록 3개(아이디어 플레이그라운드 · WWDC26 하이라이트 · Apple Korea PR 프라이빗 이벤트)를 썼고 사진 워터마크(2-9)·사진 칸 비율 옵션(5-26)까지 끝냈다. **세 글 모두 사용자 확인을 마쳤다.** 앞의 둘은 `develop` 병합 `c2ac9e0`, Apple PR 글은 **`feature/seminar-apple-pr-event`**(`b9b0039`). **바로 다음: 이 브랜치 `develop` 병합 → push → `master` 병합(배포)** — 사용자에게 확인받고 진행한다(맨 위 "새 채팅은 여기부터"). 그 뒤 새 재료가 오면 `docs/content-templates/`의 **재료 양식**(`seminar-input.md` · `study-input.md`)에 맞춰 읽어 글 틀에 채운다. Notion 링크면 **2-7**, 사진 처리는 **2-8**, 워터마크는 **2-9**. **스터디는 아직 글이 하나도 없다**
     3. **배포** — 콘텐츠가 들어가면 `develop` → `master`. 문서만 고쳤을 때는 하지 않는다
...
- **` 2`가 붙은 사본 파일(2026-09-21 원인 확인)**: 저장소가 `~/Documents` 아래에 있고 macOS의 **iCloud "데스크탑 및 문서 폴더" 동기화**가 켜져 있다(`defaults read com.apple.finder FXICloudDriveDesktop` = 1). 동기화가 충돌을 만나면 `파일 2.확장자`·`폴더 2` 사본을 만든다. 이번에는 루트·`design/`·`src/` 파일 30개(세션 도중 스스로 사라짐), 빈 폴더 9개, `node_modules` 안 612개가 있었다. `.git` 안에는 없었다
  ...
  - 2026-09-22~23에는 거의 작업마다 생겼다: 소스 몇 개, `node_modules`에 17~144개, 빌드 결과 `dist/`에 180여 개. `dist/`는 폴더째 지우면 되고(다시 빌드하면 생긴다), `node_modules`는 지우고 `npm ci`
...
- 세미나 기록 3개 + 사진 워터마크·칸 비율(2026-09-23~24)
  - `feature/seminar-idea-playground`: `a7f91be` 사진 칸 `aspect` 옵션(5-26) · `bdeae62` 아이디어 플레이그라운드 · `110ccc4` WWDC26 하이라이트 · `1b2993f` letswift 사진 워터마크·이름표 `aspect` → `develop` 병합 `c2ac9e0`, 브랜치 삭제
  - `feature/seminar-apple-pr-event`: `b9b0039` Apple Korea PR 프라이빗 이벤트(발표자는 `수료생`, 표지는 참가 메일을 잘라 재배치 — 맨 위 세션 기록)
  - 모든 세미나 사진에 `Lia.log` 워터마크(2-9). 주최 값은 세 가지(`Let'Swift` · `Apple` · `Apple 디벨로퍼 아카데미 @ POSTECH` ×2)라 목록에 주최별 필터 칩이 뜬다. 새 글도 같은 주최면 **글자를 똑같이** 적어야 한 칩으로 묶인다
...
- Pointer Quest 진행 기간 흐름(`feature/real-content`, 2026-09-23, 커밋 `7c48528`)
  ...
  - 기간 근거는 **pointer-quest 저장소 커밋 이력**: 2026-01-12 첫 커밋 → 2월 14일까지 127개 → 3~6월 0개 → 7월 15일 재개(XcodeGen 재구성) 311개 → 9월 15일 `1.0.0` 출시. `gh api repos/yuminc03/pointer-quest/commits --paginate`로 셌다
  - 확인: 빌드, 데스크톱 다크·390·768 스크린샷. 사용자에게 3~6월 표현·시작일·요약 칸 표기를 확인해 달라고 남겼다
- 세미나 양식 결정(2026-09-23, 결정은 **5-25**, 구현 대기): 주최 `host` 필드(+ 주최별 필터 칩) · 세션 한눈에 보기(본문 소제목에서 자동) · 목록 카드의 세션 수. 세션별 페이지는 쓰지 않는다. 사용자가 세미나 내용을 넣을 때 함께 구현한다
...
#### 재료를 받는 형식 — **양식 파일이 기준이다**(2026-09-23)
재료를 받는 칸은 `docs/content-templates/`의 **재료 양식**에 정리해 두었다. 사용자에게는 이 파일을 안내하고, 받은 내용을 그 칸에 맞춰 읽는다.

| 재료 양식 | 사진 | 비고 |
|---|---|---|
| `seminar-input.md` | **2~3장, 많아야 4장**(표지 1 + 본문 1~2) | 6번에 Notion으로 주는 법(2-7)과 최소 재료 네 줄 |
| `study-input.md` | **없어도 된다.** 있으면 1~2장 | 3번 "막혔던 것"이 이 글의 값어치다 |
| (프로젝트는 아직 없음) | 아이콘·스크린샷·기능 화면 | Pointer Quest는 저장소 README를 재료로 썼다 |

- **다 채우지 않아도 된다.** 꼭 필요한 칸은 **"공개하면 안 되는 것"** 하나다 — 필기만 봐서는 공개해도 되는 내용인지 Claude가 판단할 수 없다. 실제로 10 Years of Let'Swift에서 발표자의 미공개 분석 수치를 뒤늦게 뺐다(`3f569bc`)
- **소감·배운 점(세미나 4 · 스터디 4)은 선택**이다(2026-09-23 사용자 결정). 세션·본문 내용에 이미 녹아 있는 경우가 많아, 비어 있으면 Claude가 초안을 써서 보여 주고 사용자가 고친다
- 글은 채팅에 편하게, 사진은 폴더 경로로 받는다. **고르고 자르고 크기 줄이는 것은 Claude가 한다** — 후보를 넉넉히 받아도 된다
...
- [x] Pointer Quest에 진행 기간 흐름 추가(2026-09-23, 커밋 `7c48528`, 배포 `dc3db99`)
- [x] 세미나 1: 10 Years of Let'Swift(`seminars/10-years-of-letswift/`) + 세미나 양식 확장(2026-09-23, 커밋 `64f02a0`, 배포 `dc3db99`)
- [x] 세미나·스터디 **재료 양식** 추가(2026-09-23, `d2f5b26`·`769b28a` + 스터디 양식)
- [x] 세미나 2: 아이디어 플레이그라운드(`seminars/idea-playground-olive-young-n/`, `bdeae62`) · 세미나 3: WWDC26 하이라이트(`seminars/wwdc26-highlights/`, `110ccc4`) — `develop` 병합 `c2ac9e0`(2026-09-24, 배포 전)
- [x] 세미나 4: Apple Korea PR 프라이빗 이벤트(`seminars/apple-korea-pr-private-event/`, `b9b0039`, 브랜치 `feature/seminar-apple-pr-event`, 2026-09-24, 배포 전)
- [ ] 다음 프로젝트·스터디 글·세미나는 재료가 오는 대로. **스터디는 아직 글이 하나도 없다**(목록이 빈 상태로 떠 있다)

#### 배포는 이미 되어 있다 (2026-09-21)
배포를 먼저 하기로 해 사이트가 이미 떠 있다(5-20, 최근 배포 `dc3db99`). 글을 쓰면 `develop`에서 확인 → `develop` → `master` 병합으로 올린다. 실제 주소에서 보면서 실기기 확인(남은 일)도 함께 할 수 있다.
...
- **한 번에 다 오지 않는다**(2026-09-23 WWDC26 글에서 겪음). 응답의 `cursors[0].stack`을 다음 호출의 `cursor`로, `chunkNumber`를 1씩 올려 **`stack`이 빌 때까지 반복**해 `recordMap`을 합친다. chunk 0만 쓰면 뒷부분 블록이 `content`에는 있는데 `block`에 없어 통째로 빠진다(359블록짜리 글이 4번에 나눠 왔다)
- **행사 메타데이터는 본문이 아니라 DB 속성에 있을 수 있다.** 페이지가 Notion 데이터베이스의 행이면 날짜·주최·분류가 루트 블록의 `properties`에 `NgU<` 같은 임의 키로 들어 있다(스키마는 `collection` 레코드에 있는데 공개 페이지에서는 `role: none`이라 못 읽으니, 값만 보고 뜻을 짐작한 뒤 사용자에게 확인한다)
- **세션 제목·발표자는 제목 슬라이드 사진에 있다.** 필기에 없어도 각 세션 첫 사진을 받아 보면 대개 적혀 있다
...
### 2-8. 세미나 사진 넣는 방법 (2026-09-23 정리)
재료 사진을 글 폴더에 넣을 때 매번 같은 순서로 한다.

- **파일 이름**: 표지는 `cover.jpg`, 본문은 `photo-<식별어>.jpg`(예: `photo-mixer.jpg`·`photo-agenda.png`). 원본 이름(`IMG_7677.JPG`)을 그대로 쓰지 않는다 — 나중에 무슨 사진인지 알아볼 수 없다
- **크기**: 긴 변 **1600px**, JPEG 품질 82(`mozjpeg`). 스크린샷처럼 글자가 있는 것만 PNG로 둔다
- **EXIF 회전을 픽셀에 구워 넣는다.** 아이폰 사진은 픽셀이 가로로 저장되고 "세로로 돌려서 보여라"라는 EXIF 표시만 붙어 있어(`sips -g pixelWidth`가 4032×3024로 나오는데 화면에서는 세로), 그대로 줄이면 도구에 따라 방향이 달라진다. `sharp(...).rotate()`를 **resize 앞에** 붙이면 해결된다. `sharp`는 `node_modules/sharp/dist/index.mjs`에서 직접 import한다(Astro가 이미 갖고 있다)
- **표지는 반드시 가로 사진**으로 고른다. 목록 카드와 공유 카드(OG)에서 가로로 좁게 잘리므로 세로 사진은 글자가 잘린다
- **자를 것은 자른다**: 행사 등록 QR·마감일이 찍힌 초대장, 제3자의 얼굴·이름표. `sharp`의 `extract`로 비율을 지정해 자른다
- **원본은 옮긴 뒤 지운다**(2026-09-23 사용자 지시). 재료 폴더는 저장소 루트에 `seminar_*/`로 두고 글에 넣은 뒤 지운다. **`.gitignore`에는 넣지 않았으므로**(2026-09-24 사용자 결정 — `seminar`로 시작하는 진짜 폴더가 생길 수 있다) 커밋할 때 `git add`에 **경로를 지정**해 재료 폴더가 섞이지 않게 한다
- 변환 스크립트는 세션 스크래치패드에 두었다(저장소에는 두지 않는다 — 한 번 쓰고 마는 도구다)

#### sharp를 쓸 때 두 번 막힌 곳 (2026-09-24)
둘 다 "파이프라인이 언제 적용되는가"를 잘못 안 것이었다. 중간 버퍼로 한 번 끊으면 둘 다 해결된다.

- **`metadata()`는 resize 전 크기를 돌려준다.** 4032px 기준으로 만든 오버레이를 1600px 결과물에 얹으려다 `Image to composite must have same dimensions or smaller`로 실패했다
- **sharp는 호출 순서와 무관하게 resize를 composite보다 먼저 적용한다.** `.composite(...).resize(...)`라고 써도 resize가 먼저 실행되어 같은 오류가 난다
- 해결: `await pipeline.png({ compressionLevel: 0 }).toBuffer({ resolveWithObject: true })`로 **무손실 중간 버퍼**를 뽑아 실제 출력 크기를 잰 뒤, 새 `sharp(buffer)`에 오버레이를 얹는다. 이러면 JPEG은 마지막에 한 번만 손실 압축된다

### 2-9. 사진 워터마크 (2026-09-24 결정)
사용자가 "웹사이트 이미지를 함부로 가져가지 못하게 하는 방법"을 물어 정리한 결과다. **완전히 막는 방법은 없다**(브라우저가 그린 것은 저장할 수 있고, 저장소가 공개라 원본 파일도 받아진다). 그래서 막는 대신 **가져가도 출처가 남게** 하는 쪽으로 정했다. 우클릭·드래그 금지는 쓰지 않는다 — 몇 초면 우회되고 정상 사용자만 불편하다(특히 개발자가 보는 포트폴리오다).

- **모양**: 반투명 검은 판(불투명도 0.42, 둥근 모서리) + 흰 글자 `Lia.log`(0.97). **흰 글자에 검은 그림자만 주는 방식은 쓰지 않는다** — 흰 배경에서 원리적으로 사라진다. 판이 배경을 가려야 밝은 사진·어두운 사진에서 대비가 같다
- **크기는 원본 폭이 아니라 "화면에 그려지는 폭" 기준으로 정한다.** 원본 폭의 몇 %로 고정하면 작게 그려지는 칸에서 같이 작아져 안 보인다(2.6%로 고정했더니 `Photo`는 21px인데 `PhotoPair`는 10px이 됐다). **데스크톱에서 18px**이 되도록 칸마다 역산한다 — `Photo` 800px → 2.25%, `PhotoPair` 394px → 4.6%, `PhotoSide` 340px → 5.3%
- **본문 사진은 오른쪽 아래 구석**(여백은 폭의 2.2%)
- **표지만 위치가 다르다.** 표지는 모든 칸이 `object-fit: cover`로 세로 가운데만 남기므로 구석에 두면 반드시 잘린다. 그래서 **모든 칸에서 살아남는 가장 아래 지점**에 둔다 — 상세 표지 3종·목록 카드 3종·공유 카드(OG) 1200×630을 모두 계산해 가장 납작한 칸(목록 카드 데스크톱 954×280, 세로 39%만 남음)을 기준으로 삼는다. 4:3 표지는 세로 70%, 1.12:1 표지는 66% 지점이 나왔다. **숫자를 박지 말고 칸 목록에서 계산하게 둔다** — 표지가 늘어도 자동으로 맞는다
- **적용 대상**: 세미나 사진 전체(2026-09-24 기준 13장). 프로젝트 스크린샷·아이콘에는 넣지 않았다(앱 화면이라 어색하다)
- **원본을 덮어쓴다.** 워터마크 없는 판은 남기지 않는다 — 저장소가 공개라 남겨 두면 의미가 없다
...
| 사진 칸 비율 · 워터마크 | `PhotoPair`·`PhotoSide`·`Photo`에 `aspect`(5-26), 세미나 사진 전체에 `Lia.log` 워터마크(2-9) | `feature/seminar-idea-playground` | 사용자 미리보기 확인, `a7f91be`·`1b2993f`(2026-09-24) |
| 세미나 2·3 | 아이디어 플레이그라운드 · WWDC26 하이라이트 | `feature/seminar-idea-playground` | 사용자 내용 확인, `develop` 병합 `c2ac9e0`(2026-09-24, 배포 전) |
| 세미나 4 | Apple Korea PR 프라이빗 이벤트 | `feature/seminar-apple-pr-event` | 사용자 내용 확인, `b9b0039`(2026-09-24, 병합·배포 전) |
...
- [x] 스터디·세미나 예시 글·임시 이미지 삭제, 틀은 `docs/content-templates/`로(2026-09-23, 병합 `603b658`, 배포 전)
- [x] **세미나 양식 확장**: 주최 `host`·세션 한눈에 보기·카드의 세션 수(2026-09-23 결정, **5-25**) — `feature/seminar-sessions`로 구현, 배포 `dc3db99`
- [ ] 다음 배포: `develop` → `master`(세미나 3개 · 워터마크 · 사진 칸 비율이 아직 배포 전이다. 먼저 `feature/seminar-apple-pr-event`를 `develop`에 병합)
- [ ] (선택) GitHub Actions 버전 올리기 — 2026-09-22 배포 로그 안내: `actions/deploy-pages@v4`가 Node 20용(Node 24로 강제 실행), `ubuntu-latest`가 2026-10-19부터 Ubuntu 26. 지금 배포에는 문제없음
...
- **배경**: 사용자 질문 — 특정 기관이 여는 세미나는 한 행사에 세션이 여러 개인데, 읽기 쉽게 정리할 방법이 있나. 지금 구조(행사 하나 = 페이지 하나, 본문 `## SESSION 01 · 발표자` + `### 세션 제목`)의 아쉬운 점
  ...
  - **주최 기관을 적을 칸이 없다**(제목·본문에 섞어 써야 하고 주최별로 모아 볼 수 없다)
- **결정(사용자 승인)**: 행사 하나 = 페이지 하나는 유지하고 세 가지를 더한다. **구현은 사용자가 세미나 내용을 넣을 때 그 글로 확인하며 한다**
  1. **주최 필드 `host`**(선택): 프런트매터 한 줄(`host: 'Apple Developer Academy'`). 목록 카드와 상세 머리의 날짜·장소 옆에 표시. 주최가 2곳 이상이면 목록 위에 **주최별 필터 칩**(`FilterChips` 재사용 — 분류가 하나뿐이면 칩을 그리지 않는 동작이 이미 있다)
  2. **세션 한눈에 보기**: 표지 사진 아래 세션 목록(번호 · 세션 제목 · 발표자). **본문 소제목에서 자동으로 뽑아** 글쓴이가 따로 적을 것이 없다. 항목을 누르면 그 세션으로 이동
  3. **목록 카드에 "세션 N개"**
- **쓰지 않기로 한 것**: 세션마다 페이지 따로 — 세션 하나가 몇 문단이라 페이지가 너무 짧고, 행사 흐름(도입 → 세션 → 소감)이 끊기며, 목록이 세션 수만큼 길어진다
- **구현 메모**(할 때 다시 판단)
  - 스키마: `seminars`에 `host: z.string().optional()`(`src/content.config.ts`). 틀 `docs/content-templates/seminar/index.mdx`와 5-10 필드 목록도 함께 고친다
  - 세션 뽑기: `render()`의 `headings`에서 `depth 2`가 `SESSION NN · 발표자`이고 바로 다음이 `depth 3`인 쌍을 세션으로 본다(본문 CSS가 이미 `h2:has(+ h3)`로 같은 규칙을 쓴다). 번호·발표자·제목·앵커(`h2`의 slug)를 담은 배열을 만드는 순수 함수로 두고(`src/lib/`), 상세 페이지와 목록 카드가 함께 쓴다. `배운 점 · 소감`처럼 `###`가 없는 `##`는 세션이 아니다
  - 선택: 시간·트랙은 `## SESSION 01 · 김발표 · 14:00`처럼 세 번째 칸으로 받을 수 있게(없으면 생략)
  - 화면: 모든 폭에서 표지 아래 인라인 카드로 시작한다. 데스크톱에서 스터디처럼 sticky 레일(`Toc`)까지 둘지는 실제 세션 수를 보고 정한다(`Toc`는 지금 `depth 2`만 받으므로 레일로 쓰려면 항목을 직접 넘기는 prop이 필요)
...
### 5-26. 사진 칸 비율 옵션: `PhotoPair`·`PhotoSide`의 `aspect` (2026-09-24 결정·구현)
두 컴포넌트가 `aspect-ratio: 4 / 3`을 CSS에 박아 두고 `object-fit: cover`로 잘라 내고 있었다. 가로 사진 기준이라 **세로로 긴 사진을 넣으면 위아래가 잘려 나간다.** 워터마크가 안 보이는 원인을 찾다가, 아이디어 플레이그라운드 글의 **프로토타입 화면이 세로 38%만 남아 지도·아이폰 테두리·버튼이 전부 잘려 있던 것**을 발견했다. 워터마크와 무관하게 고쳐야 할 문제였다.

- **고친 방법**: `aspect-ratio: var(--shot-aspect, 4 / 3)`으로 바꾸고, `aspect` prop을 받아 인라인 `style`로 `--shot-aspect`를 내린다. **기본값이 그대로 `4 / 3`이라 기존 글은 영향받지 않는다**
- **쓰는 법**: 원본 비율에 맞는 값을 넘긴다. `<PhotoPair aspect="1 / 2" …>`, `<PhotoSide aspect="2 / 3" …>`. 안 넘기면 가로 4:3으로 잘린다
- **적용한 값**: 프로토타입 2장 `1 / 2`, 굿즈 노트·핀 `3 / 4`, hello 배너 `2 / 3`, letswift 이름표 `1120 / 636`. 넷 다 잘림 22~38% → **0%**
- **`PhotoPair`는 두 칸이 한 비율을 공유한다.** 두 사진의 원본 비율이 조금이라도 다르면 한쪽이 잘린다 — 프로토타입 2장(0.484 / 0.506)은 **검은 여백을 넣어 둘 다 정확히 800×1600으로 맞췄다**(원본 배경도 검은색이라 이어 붙는다). 비율이 다른 두 장을 나란히 둘 때는 이 방법을 쓴다
- **표지(`SeminarCover`)는 고치지 않았다.** 높이를 240/360/440px로 고정하는 것이 시안이고 목록 카드·공유 카드와 형태를 맞춰야 한다. 표지는 잘리는 것을 전제로 워터마크 위치를 잡았다(2-9)
...
| `docs/content-templates/seminar-input.md` | 세미나 재료를 넘겨받는 양식. 행사·세션·발표 외 순서·소감(선택)·사진(2~3장)·Notion으로 주는 법 + 빈 양식(2026-09-23) |
| `docs/content-templates/study-input.md` | 스터디 재료를 넘겨받는 양식. 글 정보·내용·막혔던 것·정리(선택)·사진(없어도 됨) + 빈 양식(2026-09-23) |
...
## 12. 브랜치·커밋 기록
- 로컬 브랜치: `master`, `develop`(`c2ac9e0`), **`feature/seminar-apple-pr-event`(현재)**. 원격(`origin`, `https://github.com/yuminc03/my-homepage.git`, **2026-09-21부터 공개 저장소**): `master` `dc3db99`(2026-09-23 배포, 로컬과 같음), `develop` `119d4a1`(**로컬 `develop`이 앞서 있다 — 2026-09-24 병합을 아직 push하지 않았다**). **`master`에 push하면 GitHub Actions가 배포한다**(5-20)
- `feature/seminar-apple-pr-event`(2026-09-24, `develop` `c2ac9e0`에서 분기, **병합 전**). 커밋 직전에 `npm run build`
  - `b9b0039` content: Apple Korea PR 프라이빗 이벤트 세미나 기록 추가
  - docs: 세미나 기록 4개 완성과 다음 병합·배포 순서 정리(이 문서와 `CLAUDE.md`, 커밋 해시는 `git log`로 확인 — 이 줄을 고치며 amend했다)
- `feature/seminar-idea-playground`(2026-09-23~24, `develop` `119d4a1`에서 분기, **`develop` 병합 `c2ac9e0`로 완료, 브랜치 삭제, push 안 함**)
  - `a7f91be` feat: 세미나 사진 칸에 가로세로 비율 옵션 추가
  - `bdeae62` content: 아이디어 플레이그라운드 워크숍 기록 추가
  - `ec9ceb2` docs: 워터마크·사진 칸 비율 결정과 2026-09-24 작업 정리
  - `110ccc4` content: WWDC26 하이라이트 살펴보기 세미나 기록 추가
  - `1b2993f` content: 10 Years of Let'Swift 사진에 워터마크와 칸 비율 적용
  - `a9a4096` docs: WWDC26 글 확인 결과와 재료 폴더 관리 방식 정리
- `feature/seminar-sessions`(2026-09-23, `develop` `e283cd3`에서 분기, **`develop` 병합 `b615ca9`로 완료, 브랜치 삭제·push**. 배포 `dc3db99`). 각 커밋 직전에 `npm run build`
  ...
  - `bcec834` docs: 세미나 양식 확장과 첫 세미나 기록 정리
  - `3f569bc` fix: 세미나 글에서 발표자 미공개 분석 수치 삭제
  - `d2f5b26` docs: 세미나 재료 양식 추가
  - `769b28a` docs: 세미나 재료 양식의 소감·배운 점을 선택으로 변경
...
- `develop` 직접 커밋: `92083d0` `.claude/settings.json` 권한 추가(2026-09-14), `e32bb16` docs: 스터디 목록·글 작업 계획 정리(2026-09-18), `a322ec1` docs: 페이지 전환 병합 결과와 다음 작업(콘텐츠 검색) 정리(2026-09-20), `5fa1638` docs: 콘텐츠 검색 방식 4건 확정 기록(2026-09-20), `32b70ba` docs: 콘텐츠 검색 병합 결과와 다음 작업(실제 콘텐츠) 정리(2026-09-21), `b15b3dc` docs: GitHub Pages 배포 결과와 결정 기록(2026-09-21), `48ae273` docs: 인수인계 문서의 어긋난 부분 2건 수정(2026-09-21), `a9486e7` docs: Lia.log 병합·배포 결과 기록(2026-09-21), `c63bdc4` docs: 인수인계 문서의 지난 상태 정리와 이번 세션 요약(2026-09-21), `290720a` docs: Pointer Quest·README·북마크 카드·OG 태그 배포 결과 기록(2026-09-22), (이 문서 갱신) docs: 세미나 양식 결정과 2026-09-22~23 작업 정리(2026-09-23)
