...
- **마지막 세션(2026-09-24)**: **사진 워터마크(2-9)를 넣고, 그 과정에서 사진 칸이 세로 사진을 잘라 먹는 문제(5-26)를 찾아 고쳤다.** 사용자가 "웹사이트 이미지를 함부로 못 가져가게 하는 방법"을 물어 시작했다 — 완전히 막을 수는 없다고 설명하고(저장소가 공개라 원본도 받아진다) **워터마크**로 정했다. 강도를 세 번 다시 잡았다: 흰 글자 45%(안 보임) → 흰 글자 확대(흰 배경에서 원리적 한계) → **반투명 판 + 흰 글자**. 그래도 칸마다 보였다 안 보였다 해서 원인을 파 보니 **① `PhotoPair`·`PhotoSide`·`SeminarCover`가 `object-fit: cover`로 아래를 22~31% 잘라내고 ② 워터마크를 원본 폭 기준으로 넣어 작게 그려지는 칸에서 10px까지 줄어드는 것**이었다. ①은 `aspect` prop을 만들어 고쳤고(5-26) **이 과정에서 프로토타입 화면이 세로 38%만 남아 지도·버튼이 다 잘려 있던 것을 발견해 함께 고쳤다**, ②는 칸별 표시 폭에서 역산해 모두 18px로 맞췄다. 표지는 어떤 칸에서도 구석이 잘리므로 **모든 칸에서 살아남는 가장 아래 지점**(4:3은 세로 70%, 1.12:1은 66%)에 두기로 사용자가 미리보기를 보고 정했다. 사용자 확인 완료. 이어서 재료 폴더 `seminar_resource/`를 지우고 `.gitignore`에 `/seminar_*/`를 넣었다.
  ...
  - 남은 것(작업 트리에만 있음): **WWDC26 글**(`src/content/seminars/wwdc26-highlights/`, 사용자가 내용을 더 볼 예정) · **letswift 워터마크 3장과 `index.mdx`의 `aspect`**(컴포넌트 기본값이 `4 / 3` 그대로라 안 넣어도 기존 글은 예전과 같이 나온다) · **`.gitignore`의 `/seminar_*/`**
...
3. **2026-09-23 배포 완료**: 글꼴 직접 제공(5-24) · 예시 글 정리 · Pointer Quest 진행 기간 · 세미나 양식 확장(5-25) · 첫 세미나 기록을 `develop` → `master` 병합 `dc3db99`로 한 번에 배포했다(run 35821441390 성공). 실제 주소에서 홈·세미나 목록·행사 상세·검색 색인·프로젝트 상세 200, 상세의 세션 한눈에 보기·주최·세션 수·북마크 카드·표지 OG를 확인했다. (그 전 배포는 2026-09-22 `81b7ac3`)
   ...
   - **다음에 할 일(사용자 승인 순서)**
     ...
     2. **실제 콘텐츠 채우기**(2-6) ← **지금 여기.** 세미나 기록 2개(WWDC26 하이라이트 · 아이디어 플레이그라운드)를 썼고 사진 워터마크(2-9)·사진 칸 비율 옵션(5-26)까지 끝냈다. 지금 브랜치는 **`feature/seminar-idea-playground`**, 커밋은 **아이디어 플레이그라운드 것 2개만** 되어 있다(`a7f91be`·`bdeae62`). **다음 세션은 사용자가 WWDC26 글을 확인하는 것부터 시작한다** — 확인되면 WWDC26 글 · letswift 워터마크 · `.gitignore`를 커밋하고 `develop` 병합 → 배포한다. 내용은 아이디어 플레이그라운드만 사용자 확인을 마쳤다. 다음 재료는 `seminar_apple_pr_event/`(Apple Korea PR 프라이빗 이벤트)에 이미 들어와 있다. 재료가 오면 `docs/content-templates/`의 **재료 양식**(`seminar-input.md` · `study-input.md`)에 맞춰 읽어 글 틀에 채운다. Notion 링크면 **2-7**, 사진 처리는 **2-8**, 워터마크는 **2-9**. **스터디는 아직 글이 하나도 없다**
...
- **원본은 옮긴 뒤 지운다**(2026-09-23 사용자 지시). 재료 폴더는 저장소 루트에 `seminar_*/`로 두고 글에 넣은 뒤 지운다. `.gitignore`에 `/seminar_*/`가 있어 실수로 커밋되지 않는다
