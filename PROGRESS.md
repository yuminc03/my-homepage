# 진행 상황
- 최종 업데이트: 2026-09-24 (세미나 기록 4개 완성, 모두 `develop`에 병합 `2d40dcc`, **다음은 `develop` push → `master` 배포**)
- 이 문서 하나만 읽으면 새 채팅에서 바로 이어서 작업할 수 있도록 정리한 단일 기준 문서다
- **새 채팅은 여기부터**: 지금 브랜치는 **`develop`**(`2d40dcc`, `feature/seminar-apple-pr-event` 병합 — 기능 브랜치는 남아 있지 않다)이다. 작업 트리는 깨끗하다(재료 폴더 `seminar_apple_pr_event/`는 2026-09-24 사용자 요청으로 지웠다). 원격 push는 이번 세션에 하지 않았다 — `develop`은 `origin/develop`보다 앞서 있다. **다음 할 일(사용자 확인 후): ① `develop` push ② `develop` → `master` 병합 push로 배포**(세미나 3개 · 사진 칸 비율 · 워터마크가 아직 배포 전) ③ 실제 주소에서 세미나 목록·상세 4개 확인. 그 뒤는 다음 콘텐츠(스터디는 아직 0개)
...
1. `git status`로 브랜치와 작업 트리를 확인한다
   ...
   - 로컬 `develop`에 `feature/seminar-idea-playground`(`c2ac9e0`)·`feature/seminar-apple-pr-event`(`2d40dcc`) 병합이 들어 있다. **`origin/develop`(`119d4a1`)보다 앞서 있고 push하지 않았다.** `origin/master` = `dc3db99`(2026-09-23 배포)
...
## 12. 브랜치·커밋 기록
- 로컬 브랜치: `master`, **`develop`(현재)**. 기능 브랜치는 없다. 원격(`origin`, `https://github.com/yuminc03/my-homepage.git`, **2026-09-21부터 공개 저장소**): `master` `dc3db99`(2026-09-23 배포, 로컬과 같음), `develop` `119d4a1`(**로컬 `develop`이 앞서 있다 — 2026-09-24 병합을 아직 push하지 않았다**). **`master`에 push하면 GitHub Actions가 배포한다**(5-20)
- `feature/seminar-apple-pr-event`(2026-09-24, `develop` `c2ac9e0`에서 분기, **`develop` 병합 `2d40dcc`로 완료, 브랜치 삭제, push 안 함**). 커밋 직전에 `npm run build`
